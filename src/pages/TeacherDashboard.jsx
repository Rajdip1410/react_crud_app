...
export const TeacherDashboard = () => {
  ...
  const api = import.meta.env.VITE_TEACHER_API_ENDPOINT;
  ...
  const fetchStudentData = async () => {
    try {
      ...
      if (response.data.meta.code === 1) {
        ...
        console.log(response.data.meta.pagination.totalPage);
  ...
  const handleconformdelete = async (id) => {
    try {
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 font-sans text-slate-600 w-full">
      <div className="text-center py-1.5 font-bold text-3xl text-slate-800 h-12 bg-green-100">
        Teacher data
      </div>
      <div className="w-full space-y-0">
        ...
        <div className="sticky top-0 z-40 w-full overflow-hidden bg-white border-b border-slate-200/60 p-6 shadow-xs flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="z-10">
            ...
            <p className="text-sm text-slate-400 mt-1 font-medium">
              Manage, update, and monitor registered teacher profiles.
            </p>
          </div>

          {userRole === "admin" && (
            <div className="flex items-center gap-3 z-10 ml-190">
              <button
        ...
        <div className="w-full bg-white border-b border-slate-200/70 shadow-sm overflow-hidden">
          {/* Scroll Viewport Window - Dynamic height tracking while enforcing full horizontal span */}
          <div className="overflow-x-auto max-h-[calc(100vh-200px)] w-full block ">
            <table className="w-full border-collapse text-left text-sm text-slate-600 table-auto">
              <thead className="text-[16px] font-bold uppercase tracking-widest text-slate-800 w-full">
                <tr className="border-b border-slate-300/80">
                  <th className="sticky top-0 z-20 bg-slate-50 px-6 py-4">
                    SR NO.
                  </th>
                  <th className="sticky top-0 z-20 bg-slate-50 px-6 py-4">
                    ID
        ...
        <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-md flex items-center justify-center z-50 p-4">
          <div className="bg-white p-6 rounded-2xl shadow-2xl max-w-sm w-full text-center border border-slate-100 animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="flex justify-center gap-3">
              ...
              <button
                onClick={() => handleconformdelete(deleteid)}
